...
import org.salemrobotics.frc.util.LoggedProfiledPID;
...
/**
 * A command that drives in a straight line to a target pose. This command intentionally ignores obstacles; in the case
 * where the driving scenario is complex (such as defense robots or many game pieces in the way), this command should
 * be disregarded in favor of the driver navigating themselves.
 *
 * <p>The PID values for each instance for this command can be found on SmartDashboard if
 * {@link org.salemrobotics.frc.Constants#DEVBOT} is true. This command also logs a large amount of control information
...
public class DriveToPoseCommand extends Command {
     ...
     * @param poseSupplier      The supplier of the target to approach. This is called once during this command's
     *                          initialization each time it is scheduled.
     ...
     */
    public DriveToPoseCommand(SwerveDrive drive, String name, PoseSupplier poseSupplier, DoubleSupplier xOffsetSupplier, DoubleSupplier yOffsetSupplier) {
        setName(name);
        this.logKey = "AutoAlign/" + name;

        translationController = new LoggedProfiledPID(
                logKey + "/TranslationPID",
                DRIVE_KP, DRIVE_KI, DRIVE_KD,
                DRIVE_MAX_VELOCITY, DRIVE_MAX_ACCELERATION);
        translationController.setGoal(0);
        translationController.setTolerance(TRANSLATION_TOLERANCE);

        rotationController = new LoggedProfiledPID(
                logKey + "/RotationPID",
                TURN_KP, TURN_KI, TURN_KD,
                TURN_MAX_VELOCITY, TURN_MAX_ACCELERATION);
        ...
        this.driverOffset = () -> {
            var translation = DriveCommands.getLinearVelocityFromJoysticks(
                    xOffsetSupplier.getAsDouble(),
                    yOffsetSupplier.getAsDouble())
    ...
    public DriveToPoseCommand(SwerveDrive drive, String name, PoseSupplier poseSupplier) {
        ...
        translationController = new LoggedProfiledPID(logKey + "/TranslationConstants", DRIVE_KP, DRIVE_KI, DRIVE_KD, DRIVE_MAX_VELOCITY, DRIVE_MAX_ACCELERATION);
        translationController.setGoal(0);
        translationController.setTolerance(TRANSLATION_TOLERANCE);

        rotationController = new LoggedProfiledPID(logKey + "/RotationConstants", TURN_KP, TURN_KI, TURN_KD, TURN_MAX_VELOCITY, TURN_MAX_ACCELERATION);
        rotationController.enableContinuousInput(-Math.PI, Math.PI);
        rotationController.setTolerance(ROTATION_TOLERANCE);
    ...
    public void execute() {
        ...
        // The PID controller does NOT account for direction and the magnitude is assumed to be in the X direction
        ...
        val commandedTranslation = new Translation2d(linearMagnitude, errorVector.getAngle())
                .plus(driverOffset.get());
        ...
        val omega = rotationController.calculate(drivePose.getRotation().getRadians(), targetRotation.getRadians());
        Logger.recordOutput(logKey + "/AngularOutput", omega);

        // Send the command to the drivetrain
        drive.runFieldCentric(
                new ChassisSpeeds(
                    commandedTranslation.getX(),
                    commandedTranslation.getY(),
                    omega));
    }
